    ...
    if (button && !button._janitorAttached) {
      ...
      button.addEventListener("click", () => {
        ...
        setTimeout(() => {
          ...
          // You must manually map this to a valid SC track URL for now
          const soundCloudTrackUrl = prompt("🔗 Paste full SoundCloud URL for:\n" + song);
          if (!soundCloudTrackUrl) return log("❌ No URL provided.");

          log("🎵 Queued: " + soundCloudTrackUrl);
          queue.push(soundCloudTrackUrl);
          played.add(song);
          renderQueue();
          if (queue.length === 1) loadSoundCloudPlayer(soundCloudTrackUrl);
        }, 1000);
      });
    }
  };
